function filterComments (feedbackList, state, rating, selectedClinicians) {
    ...
    if (state.byRating == false && state.byClinician == false) {
        ...
        if (state.byClinician === true) {
            let physiologistFeedback = feedbackList.filter(item =>  selectedClinicians.includes(item.physiologist));
            let doctorFeedback = feedbackList.filter(item =>  selectedClinicians.includes(item.doctor));
            let physiotherapistFeedback = feedbackList.filter(item =>  selectedClinicians.includes(item.physiotherapist));
